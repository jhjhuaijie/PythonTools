...
- [x] 批量添加标签；子标签等级不受限制，可以添加一级子标签，二级子标签，也可以添加N级子标签
...
1. 遍历所有的一级标签
2. 每个一级标签调用recursion_add_sub_bookmark函数创建标签并判断是否含有子标签
3. 若含有子标签则递归，否则退出

# 对于影印版PDF文件的标题提取

> 影印版不能直接黏贴复制，需要使用<font color=red>PDF编辑器</font>
>
> 我的处理是使用福昕编辑器
>
> 1. 提取，先把含有标题的页提取为一个PDF文件；【页面管理】-> 【提取】
> 2. 转换，把PDF文件转换成一个文本文件；【转换】 ->【到其它格式】